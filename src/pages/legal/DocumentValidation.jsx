...
const DocumentValidation = () => {
  ...
  const handleReject = id => {
    setDocuments(documents.map(doc => doc.id === id ? { ...doc, status: 'Invalid', issues: ['Rejected by legal team'] } : doc));
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">Review and validate submitted documents</p>
        </div>
        <button
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filtered.map(doc => (
                <tr key={doc.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 text-sm text-gray-900">{doc.client}</td>
                  <td className="px-6 py-4 text-sm text-gray-500">{doc.uploadDate}</td>
                  ...
                  <td className="px-6 py-4 text-sm font-medium">
                    {doc.status === 'Pending' ? (
                      ...
                      <button onClick={() => navigate(`/legal/documents/${doc.id}`)} className="text-indigo-600 hover:text-indigo-900">View Details</button>
                    )}
                  </td>
                </tr>
              ))}
        ...
        <div className="sm:hidden space-y-4">
          {filtered.map(doc => (
            <div key={doc.id} className="border border-gray-200 rounded-lg p-4 bg-white shadow-sm">
              ...
              <p className="text-sm text-gray-500"><strong>ID:</strong> {doc.id}</p>
              <p className="text-sm text-gray-500"><strong>Type:</strong> {doc.type}</p>
              <p className="text-sm text-gray-500"><strong>Client:</strong> {doc.client}</p>
              <p className="text-sm text-gray-500"><strong>Upload Date:</strong> {doc.uploadDate}</p>
              <p className="text-sm text-gray-500"><strong>Issues:</strong> {doc.issues.length > 0 ? doc.issues.join(', ') : 'No issues'}</p>
              <div className="mt-2 flex flex-wrap gap-2 text-sm">
                {doc.status === 'Pending' ? (
                  <>
                    <button onClick={() => handleValidate(doc.id)} className="text-green-600 hover:text-green-900">Validate</button>
                    <button onClick={() => handleReject(doc.id)} className="text-red-600 hover:text-red-900">Reject</button>
                  </>
                ) : (
                  <button onClick={() => navigate(`/legal/documents/${doc.id}`)} className="text-indigo-600 hover:text-indigo-900">View Details</button>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
        <h3 className="text-lg font-semibold text-blue-900 mb-2">Document Guidelines</h3>
